...
contract("NestMining", async accounts => {
    it('test', async () => {
        ...
        */
        ...
        let list = await nestVote.list(0, 3, 0);
        for (var i in list) {
            console.log(list[i]);
        }
        ...
        let setQueryPrice = await SetQueryPrice.new(nestGovernance.address);
        console.log('setQueryPrice: ' + setQueryPrice.address);
        

        await nest.approve(nestVote.address, ETHER('2000000000'));
        await nestVote.vote(2, ETHER('2000000000'));
